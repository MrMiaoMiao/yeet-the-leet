#
# @lc app=leetcode id=1500 lang=ruby
#
...
# list of IDs of all the users who own this chunk. If the user receive a
...
# m[i], what are the changes you have to do?
# 
# 
# 
# Example:
...
# 
# fileSharing.request(1, 3);   // The user with id = 1 requested the third file
# chunk, as only the user with id = 2 has the file, return [2] . Notice that
...
# 
# fileSharing.leave(1);        // The user with id = 1 left the system, all the
# file chunks with them are no longer available for other users.
# 
# fileSharing.request(2, 1);   // The user with id = 2 requested the first file
# chunk, no one in the system has this chunk, we return empty list [].
# 
# fileSharing.leave(2);        // The user with id = 2 left the system.
# 
# fileSharing.join([]);        // A user who doesn't have any chunks joined the
# system, assign id = 1 to them and return 1. Notice that ids 1 and 2 are free
