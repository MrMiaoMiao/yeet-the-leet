#
# @lc app=leetcode id=1516 lang=ruby
#
...
# If p is already a direct child of q, don't change anything. Node p must be
# the last child in the children list of node q.
# 
...
# sub-tree of node q. We move node p with its sub-tree to be a direct child of
# node q.
...
# sub-tree of node q and vice-versa. We can move node 3 with its sub-tree and
# make it as node 8's child.
# 
# 
# Example 4:
# 
...
# The first step, we move node p (with all of its sub-tree except for node q)
# and add it as a child to node q.
# Then we will see that the tree is disconnected, you need to reconnect node q
# to replace node p as shown.
# 
# 
# Example 5:
# 
...
# The first step, we move node p (with all of its sub-tree except for node q)
# and add it as a child to node q.
# As node p was the root of the tree, node q replaces it and becomes the root
# of the tree.
# 
# 
# 
